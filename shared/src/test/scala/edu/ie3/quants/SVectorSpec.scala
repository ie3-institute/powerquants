...
import edu.ie3.quants
import edu.ie3.quants.space.{Degrees, Kilometers, SquareKilometers, SquareMeters}
...
class SVectorSpec extends AnyFlatSpec with Matchers {
  ...
  it should "create a Vector with expected values from Polar coordinates" in {
    val vector = quants.SVector(10, Degrees(45))
  ...
  it should "create a Vector with expected values" in {
    val vector = quants.SVector(Kilometers(1), Kilometers(10), Kilometers(5))
  ...
  it should "create a Vector with expected values from Polar coordinates" in {
    val vector = quants.SVector(Kilometers(10), Degrees(45))
  ...
  it should "equate to like Vectors" in {
    ...
    val y = Kilometers(2)
    val z = Kilometers(3)
    quants.SVector(x, y, z).equals(quants.SVector(x, y, z)) should be(right = true)
    quants.SVector(x, y, z) == quants.SVector(x, y, z) should be(right = true)
  }
  ...
  it should "not equate to dislike Vectors" in {
    ...
    val y = Kilometers(2)
    val z = Kilometers(3)
    quants.SVector(x, y, z).equals(quants.SVector(z, y, z)) should be(right = false)
    quants.SVector(x, y, z) != quants.SVector(z, y, z) should be(right = true)
  }

  it should "determine a magnitude" in {
    val vector = quants.SVector(Kilometers(.003), Kilometers(.004), Kilometers(.005))
  ...
  it should "normalize a Vector" in {
    ...
    val y = Kilometers(4)
    val z = Kilometers(5)
    val normalized = quants.SVector(x, y, z).normalize(Kilometers)
  ...
  it should "map to a DoubleVector" in {
    ...
    val y = Kilometers(2)
    val z = Kilometers(3)
    val quantityVector = quants.SVector(x, y, z)
  ...
  it should "map to a QuantityVector" in {
    ...
    val z = Kilometers(3)
    val quantityVector = quants.SVector(x, y, z)
    quantityVector.map[Area](l ⇒ l * l).equals(quants.SVector(SquareKilometers(1), SquareKilometers(4), SquareKilometers(9))) should be(right = true)
    quantityVector.map[Length](_ * 2).equals(quants.SVector(Kilometers(2), Kilometers(4), Kilometers(6))) should be(right = true)
  }
  ...
  it should "add two Vectors" in {
    ...
    val z = Kilometers(3)
    val a = Kilometers(5)
    quants.SVector(x, y, z).plus(quants.SVector(a, a, a)) should be(quants.SVector(x + a, y + a, z + a))
  }
  ...
  it should "subtract two Vectors" in {
    ...
    val z = Kilometers(3)
    val a = Kilometers(5)
    quants.SVector(x, y, z).minus(quants.SVector(a, a, a)) should be(SVector(x - a, y - a, z - a))
  }
  ...
  it should "rescale a Vector" in {
    ...
    val z = Kilometers(3)
    val r = 5d
    quants.SVector(x, y, z).times(r) should be(SVector(x * r, y * r, z * r))
    quants.SVector(x, y, z) * r// should be(SVector(x * r, y * r, z * r))
    quants.SVector(x, y, z).divide(r) should be(SVector(x / r, y / r, z / r))
    quants.SVector(x, y, z) / r should be(SVector(x / r, y / r, z / r))
  }
  ...
  it should "times and divide with another Quantity" in {
    ...
    quants.SVector(x, y, z).times(_ * r) should be(SVector[Volume](x * r, y * r, z * r))
    quants.SVector(x, y, z).divide(_ / r) should be(SVector[Length](x / r, y / r, z / r))

    quants.SVector(x, y, z).divide(rs) should be(quants.SVector(x / rs, y / rs, z / rs))
    quants.SVector(x, y, z) / rs should be(quants.SVector(x / rs, y / rs, z / rs))
  }
  ...
  it should "dot product with a DoubleVector" in {
    ...
    val a = 5d
    val expRes = x * a + y * a + z * a
    quants.SVector(x, y, z).dotProduct(SVector(a, a, a)) should be(expRes)
    quants.SVector(x, y, z) * SVector(a, a, a) should be(expRes)
  }
  ...
  it should "dot product with another QuantityVector" in {
    ...
    quants.SVector(x, y, z).dotProduct[Length, Area](quants.SVector(x, y, z), _ * _) should be(expRes)
  }
  ...
  it should "cross product with DoubleVector (3 coordinates each)" in {
    ...
    quants.SVector(x, y, z).crossProduct(SVector(a, a, a)) should be(expRes)
    quants.SVector(x, y, z) #* SVector(a, a, a) should be(expRes)

    val up = quants.SVector(Kilometers(1), Kilometers(2), Kilometers(3))
    val left = SVector(3, 2, 1)
    val forward = up crossProduct left
    val back = left crossProduct up

    forward should be (QuantityVector(Kilometers(-4), Kilometers(8), Kilometers(-4)))
    back should be (quants.QuantityVector(Kilometers(-4), Kilometers(8), Kilometers(-4)))
  }
  ...
  it should "cross product with another QuantityVector (3 coordinates each)" in {
    ...
    val expRes = SVector(
      ...
      z * a - x * a,
      x * a - y * a)
    quants.SVector(x, y, z).crossProduct[Length, Area](quants.SVector(a, a, a), _ * _) should be(expRes)
    ...
    forward should be (quants.QuantityVector(Kilometers(-4), Kilometers(8), Kilometers(-4)))
    back should be (quants.QuantityVector(Kilometers(-4), Kilometers(8), Kilometers(-4)))
  }
