...
import edu.ie3.quants.thermal.{Celsius, Fahrenheit}
import edu.ie3.quants.time.{Hertz, Hours, Minutes}
...
class QuantitySpec extends AnyFlatSpec with Matchers with CustomMatchers with TryValues {
  ...
  object Thingee extends Dimension[Thingee] {
    private[quants] def apply[A](n: A, unit: ThingeeUnit)(implicit num: Numeric[A]) = new Thingee(num.toDouble(n), unit)
  ...
  it should "Parse a String into a Quantity based on the supplied Type parameter" in {
    import edu.ie3.quants.mass.Mass
